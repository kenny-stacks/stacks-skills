...
must_haves:
  truths:
    - "Reference file documents Clarity coding style patterns (sequential asserts, meaningful errors)"
    - "Reference file documents storage optimization patterns (hash storage, minimize on-chain)"
    - "Reference file documents upgradability patterns (modular architecture, dynamic principals)"
    - "Reference file includes auto-fix guidance for common violations"
  artifacts:
...
From CONTEXT.md - Claude auto-fixes mechanical violations and explains changes:
...
### Ask Before Fixing:
- Restructuring nested if → sequential asserts (may change execution order)
